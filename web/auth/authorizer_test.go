...
func TestAuthorizer(t *testing.T) {
	...
	testCases := []struct {
		...
		{
			Alias:           "success - multiple roles",
			Path:            "/",
			AuthorizedRoles: []string{"users", "guests"},
			RequestMethod:   http.MethodGet,
			RequestHeaders: map[string]string{
				hCookie: getCookieString(&model.SessionToken{
					ID:       "id",
					Username: "demouser",
					Name:     "Demo SessionToken",
					Roles:    []string{"guests", "users"},
		...
		{
			Alias:           "success - one role",
			Path:            "/",
			AuthorizedRoles: []string{"users"},
			RequestMethod:   http.MethodGet,
			RequestHeaders: map[string]string{
				hCookie: getCookieString(&model.SessionToken{
					ID:       "id",
		...
		{
			Alias:           "forbidden - user has wrong role",
			Path:            "/",
			AuthorizedRoles: []string{"users"},
			RequestMethod:   http.MethodGet,
			RequestHeaders: map[string]string{
		...
		{
			Alias:           "forbidden - user has nil roles",
			Path:            "/",
			AuthorizedRoles: []string{"users"},
			RequestMethod:   http.MethodGet,
			RequestHeaders: map[string]string{
		...
		{
			Alias:           "forbidden - user has no roles",
			Path:            "/",
			AuthorizedRoles: []string{"users"},
			RequestMethod:   http.MethodGet,
			RequestHeaders: map[string]string{
	...
	for _, testCase := range testCases {
		...
		testFn := func(tt *testing.T) {
			...
			handler.Use(auth.NewAuthorizer(testCase.AuthorizedRoles...))
			handler.Get("/", sendMessageHandler(http.StatusOK))
