...
func TestChangePassword(b *testing.T) {
	...
	demouser2 := func() *model.User {
		...
		if err := u.SetPassword("demouser2"); err != nil {
			b.Fatal(err)
		}
		return u
	}
	...
	testCases := []struct {
		Alias           string
		Path            string
		OutputResponse  model.ChangePasswordResponse
	...
	for _, testCase := range testCases {
		...
		testFn := func(t *testing.T) {
			...
			us := &UserServiceMock{
				ChangePasswordResponse: testCase.OutputResponse,
			}

			handler := chi.NewRouter()
			handler.Use(auth.NewAuthenticator(testPassword)...)
			handler.Post("/changepassword", user.ChangePassword(us, testPassword))
