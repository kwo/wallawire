...
func TestChangeProfile(b *testing.T) {
	...
	testCases := []struct {
		Alias           string
		Path            string
		OutputResponse  model.ChangeProfileResponse
	...
	}{
		{
			...
			RequestBody:    []byte(`{"displayname": "demouser2", "password": "demouser"}`),
		...
		},
		{
			...
			RequestMethod:  http.MethodPost,
			RequestHeaders: nil,
			RequestBody:    []byte(`{"displayname": "demouser2", "password": "demouser2"}`),
	...
	for _, testCase := range testCases {
		...
		testFn := func(t *testing.T) {
			...
			us := &UserServiceMock{
				ChangeProfileResponse: testCase.OutputResponse,
			}

			handler := chi.NewRouter()
			handler.Use(auth.NewAuthenticator(testPassword)...)
			handler.Post("/changeprofile", user.ChangeProfile(us, testPassword))
